	...
	<div class="text-center">
		...
		if(request.getParameter("search_terms")!=null) {
			...
			if((result != null) && (result.size() != 0)){
				int similarPageID = result.get(0).getKey();
				String newString =  request.getParameter("search_terms");
            	...
            	HashMap<String, Integer> similarWordScores = new HashMap<String, Integer>();
	            while (similarIT.hasNext()) {
	                Map.Entry similarPair = (Map.Entry) similarIT.next();
	                Integer similarWordID = (Integer) similarPair.getKey();
	                ArrayList<Integer> similarLocations = (ArrayList<Integer>) similarPair.getValue();
	                //todo convert wordID to word
	                //out.println("(" + indexer.getWordfromID(wordID) + " , " + locations.size() + ") ");
	                similarWordScores.put(indexer.getWordfromID(similarWordID), similarLocations.size());
	            }

	            Map<String, Integer> similarSortedWordScores = indexer.sortByValue(similarWordScores);
	            Iterator similarIWords = similarSortedWordScores.entrySet().iterator();
	            int similarCount = 0;
	            while(similarIWords.hasNext() && similarCount < 5){
	            	Map.Entry similarPair = (Map.Entry) similarIWords.next();
	            	//out.println("(" + similarPair.getKey() + ", " + similarPair.getValue() + ")");
	            ...
	            indexer.finalise();
	            se = new Searcher();
	            result = se.search(newString);
	            se.finalise();
	            indexer = new Indexer();
	            out.println("<h2 class='query'> Search Results for query and similar keywords: <b>"+ newString+ "</b></h2>");	
